...
export class HttpErrorInterceptor implements HttpInterceptor {
  ...
  constructor(private nzMessageService: NzMessageService, private modal: NzModalService, private router: Router) {}
    ...
    return next.handle(req).pipe(
      ...
      finalize(() => {
        ...
        if (success) {
          ...
          if (result.code && result.msg && result.code.toString().slice(0, 1) !== "2" && result.code !== "0") {
          ...
          if (specialErrorHandle) {
            ...
            mess = errorResponse.error.message || this.status[errorResponse.status];
          }
        }
        if (mess) {
